...
#     → chiama Codestral → parsa i FILE: blocks → committa il fix
#  4. Ripete fino a max_attempts
#  5. In ogni caso ritorna { passed:, attempts:, last_output: }
#
  ...
  class TestFixLoop
    ...
    def run
      ...
      loop do
        ...
        Calvin::LOG.info "TestFixLoop attempt #{attempts + 1}/#{@max_attempts}: #{result[:success] ? 'PASS' : 'FAIL'}"

        return { passed: true, attempts: attempts, last_output: last_output } if result[:success]

        attempts += 1
        return { passed: false, attempts: attempts, last_output: last_output } if attempts >= @max_attempts

        fix_and_commit(result[:failures], attempts)
      end
    end
    ...
    def fix_and_commit(failures, attempt_number)
      Calvin::LOG.info "TestFixLoop: costruisco prompt fix per #{failures.size} failure(s)"
